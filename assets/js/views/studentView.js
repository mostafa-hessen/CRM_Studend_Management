...
export const StudentView = {
  renderTable(students) {
    ...
    tbody.innerHTML = students.map((s, i) => `
      <tr>
        ...
        <td class="text-sm text-slate-600 font-mono">${s.parent_phone || '—'}</td>
        <td><span class="text-xs bg-slate-100 text-slate-700 px-2 py-1 rounded-lg font-semibold">${ClassModel.getById(s.grade_id)?.name || s.grade_id || '—'}</span></td>
        <td>${UIService.getEducationTypeBadge(s.education_type || 'عام')}</td>
  ...
  getFormData() {
    return {
      ...
      grade_id: document.getElementById('s-class').value || null,

      education_type: document.getElementById('s-education-type').value,
      school: document.getElementById('s-school').value.trim(),
      notes: document.getElementById('s-notes').value.trim()
    };
  },

  showModal(title, student = null, classes = []) {
    ...
    if (student) {
        document.getElementById('save-student-btn-text').textContent = 'حفظ التعديلات';
        document.getElementById('s-name').value = student.name || '';
        document.getElementById('s-phone').value = student.phone || '';
        document.getElementById('s-parent-phone').value = student.parent_phone || '';
        document.getElementById('s-class').value = student.grade_id || '';
        document.getElementById('s-education-type').value = student.education_type || 'عام';
        document.getElementById('s-school').value = student.school || '';
        document.getElementById('s-notes').value = student.notes || '';
    } else {
        document.getElementById('save-student-btn-text').textContent = 'حفظ الطالب';
